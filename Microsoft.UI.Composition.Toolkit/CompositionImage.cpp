	...
	int pixelHeight)
{
	...
	image->DrawBitmapOnSurface(pixels, size);
...
HRESULT CompositionImage::DrawBitmapOnSurface(const Array<byte>^ buffer, SIZE size)
{
	HRESULT hr = S_OK;
	...
	D2D1_SIZE_U d2d1Size;

	d2d1Size.width = size.cx;
	d2d1Size.height = size.cy;
	...
	d2d1Rect.right = static_cast<float>(d2d1Rect.left + size.cx);
	d2d1Rect.bottom = static_cast<float>(d2d1Rect.top + size.cy);
